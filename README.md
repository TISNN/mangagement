...
- **功能规划（全球教授库）**：新增文档 `docs/global-professor-directory.md`，规划“教授总览 / 教授详情 / 收藏清单”三大页面与 Supabase 表结构，强化博士申请场景下的导师筛选（招生年份、奖学金、招生要求、历史去向等）。下一步需确认优先接入的国家院校名单、与申请工作台的联动方式，以及教授数据的更新频率与监控策略。
- **功能上线（全球教授库）**：实现 `/admin/professor-directory` 页面，拆分 `DoctoralFiltersPanel`、`ProfessorGrid`、`ProfessorDetailDrawer`、`ShortlistPanel`、`ProfessorMatchDrawer` 等组件，内置 12 位博士导师样例数据。支持“先选国家、再选院校、最后精细到研究方向”的筛选体验，以及资助/入学季筛选、匹配度排序、收藏清单、导出 CSV、保存筛选方案、详情抽屉、加入学生方案等核心交互，为博士申请阶段的导师匹配提供一站式体验。
