...
type TabId = 'content' | 'bundles' | 'pricing' | 'analytics';
...
const TABS: { id: TabId; label: string; icon: React.ComponentType<{ className?: string }> }[] = [
  ...
  { id: 'bundles', label: '课程包', icon: Layers },
  { id: 'pricing', label: '定价策略', icon: Receipt },
  ...
  const renderBundlesTab = () => (
    <div className="space-y-6">
      ...
      <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
        {BUNDLES.map((bundle) => (
          <div key={bundle.id} className="rounded-xl border border-gray-200 bg-gray-50 p-5 shadow-sm dark:border-gray-700 dark:bg-gray-800/40">
            <div className="flex flex-wrap items-center justify-between gap-2 mb-3">
              <div className="text-base font-semibold text-gray-900 dark:text-white">{bundle.name}</div>
              <span className={`rounded-full px-2 py-0.5 text-[11px] font-semibold ${bundleBadge[bundle.status]}`}>{bundle.status}</span>
            </div>
            <p className="text-sm leading-5 text-gray-500 dark:text-gray-400 mb-4">{bundle.description}</p>
            <div className="grid grid-cols-2 gap-3 text-xs text-gray-500 dark:text-gray-400 mb-4">
              <div className="inline-flex items-center gap-1"><BookOpenCheck className="h-3 w-3" /> 内容 {bundle.docs} 篇</div>
              <div className="inline-flex items-center gap-1"><Crown className="h-3 w-3" /> 订阅 {bundle.subscribers}</div>
              <div className="inline-flex items-center gap-1"><Coins className="h-3 w-3" /> 定价 {bundle.price}</div>
              <div className="inline-flex items-center gap-1"><Star className="h-3 w-3" /> 评分 {bundle.rating.toFixed(1)}</div>
            </div>
            <div className="flex flex-wrap items-center gap-2 text-xs">
              <button className="inline-flex items-center gap-1 rounded-lg border border-gray-200 px-2 py-1 hover:border-indigo-200 hover:text-indigo-600 dark:border-gray-600 dark:hover:border-indigo-500 dark:hover:text-indigo-300">
                <ListChecks className="h-3.5 w-3.5" /> 配置学习路径
              </button>
              <button className="inline-flex items-center gap-1 rounded-lg border border-gray-200 px-2 py-1 hover:border-emerald-200 hover:text-emerald-600 dark:border-gray-600 dark:hover:border-emerald-500 dark:hover:text-emerald-300">
    ...
    <div className="space-y-6">
      ...
      <div className="grid gap-4 md:grid-cols-2">
        {PRICING_RULES.map((rule) => (
          <div key={rule.id} className="rounded-xl border border-gray-200 bg-gray-50 p-5 shadow-sm dark:border-gray-700 dark:bg-gray-800/40">
            <div className="flex flex-wrap items-center justify-between gap-2 mb-2">
              <div className="text-base font-semibold text-gray-900 dark:text-white">{rule.name}</div>
            ...
            <div className="text-xs text-gray-400 dark:text-gray-500 mb-2">适用范围：{rule.scope}</div>
            <p className="text-sm leading-5 text-gray-500 dark:text-gray-400 mb-3">{rule.detail}</p>
      ...
      <div className="grid gap-6 md:grid-cols-3">
        <div className="rounded-2xl border border-gray-200 bg-white p-5 shadow-sm dark:border-gray-700 dark:bg-gray-800/60">
          <div className="flex items-center justify-between mb-4">
            <div>
              <h3 className="text-base font-semibold text-gray-900 dark:text-white">收益概览</h3>
              <p className="text-xs text-gray-500 dark:text-gray-400">本月实时 GMV、付费转化与退款率</p>
            </div>
            <button className="inline-flex items-center gap-1 rounded-lg border border-gray-200 px-2.5 py-1 text-xs text-gray-600 hover:border-indigo-200 hover:text-indigo-600 dark:border-gray-600 dark:text-gray-300 dark:hover:border-indigo-500 dark:hover:text-indigo-300">
        ...
        <div className="rounded-2xl border border-gray-200 bg-white p-5 shadow-sm dark:border-gray-700 dark:bg-gray-800/60">
          <div className="flex items-center justify-between mb-4">
            <div>
              <h3 className="text-base font-semibold text-gray-900 dark:text-white">受众分布</h3>
              <p className="text-xs text-gray-500 dark:text-gray-400">按角色与地域拆分访问与付费情况</p>
            </div>
            <button className="inline-flex items-center gap-1 rounded-lg border border-gray-200 px-2.5 py-1 text-xs text-gray-600 hover:border-indigo-200 hover:text-indigo-600 dark:border-gray-600 dark:text-gray-300 dark:hover:border-indigo-500 dark:hover:text-indigo-300">
          ...
          <div className="space-y-2">
            <div className="rounded-lg border border-gray-200 bg-gray-50 p-3 dark:border-gray-700 dark:bg-gray-800/40">
              ...
              <div className="mt-1 text-xs text-gray-500 dark:text-gray-400">热门：语言培训、申请工具；付费转化 18%</div>
            </div>
            <div className="rounded-lg border border-gray-200 bg-gray-50 p-3 dark:border-gray-700 dark:bg-gray-800/40">
              <div className="flex items-center justify-between text-sm font-semibold text-gray-900 dark:text-white">
                <span>顾问 / 内部员工</span>
              ...
              <div className="mt-1 text-xs text-gray-500 dark:text-gray-400">热门：案例库、服务 SOP；付费转化 29%</div>
            </div>
            <div className="rounded-lg border border-gray-200 bg-gray-50 p-3 dark:border-gray-700 dark:bg-gray-800/40">
              <div className="flex items-center justify-between text-sm font-semibold text-gray-900 dark:text-white">
                <span>B 端合作伙伴</span>
              ...
              <div className="mt-1 text-xs text-gray-500 dark:text-gray-400">热门：企业培训、授权资料；平均客单 ¥9,800</div>
            </div>
          </div>
        </div>
        ...
        <div className="rounded-2xl border border-gray-200 bg-white p-5 shadow-sm dark:border-gray-700 dark:bg-gray-800/60">
          <div className="flex items-center justify-between mb-4">
            <div>
              <h3 className="text-base font-semibold text-gray-900 dark:text-white">活动与优惠</h3>
              <p className="text-xs text-gray-500 dark:text-gray-400">管理优惠券、限时折扣与推广渠道</p>
            </div>
            <button className="inline-flex items-center gap-1 rounded-lg border border-gray-200 px-2.5 py-1 text-xs text-gray-600 hover:border-indigo-200 hover:text-indigo-600 dark:border-gray-600 dark:text-gray-300 dark:hover:border-indigo-500 dark:hover:text-indigo-300">
  ...
  return (
    <div className="space-y-6">
      <header className="space-y-4">
        <button
          onClick={() => navigate('/admin/knowledge-hub/market')}
          className="inline-flex items-center gap-2 rounded-xl border border-gray-200 px-3 py-2 text-sm text-gray-600 hover:border-indigo-200 hover:text-indigo-600 dark:border-gray-600 dark:text-gray-300 dark:hover:border-indigo-500 dark:hover:text-indigo-300"
        >
          <ArrowLeft className="h-4 w-4" />
          返回
        </button>
        <div className="space-y-1">
          <h1 className="text-3xl font-bold tracking-tight text-gray-900 dark:text-white">我发布的</h1>
          <p className="text-sm text-gray-500 dark:text-gray-400">管理您发布的知识花园内容、课程包与定价策略</p>
        </div>
      </header>
