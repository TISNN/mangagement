...
// 设置CSV文件保存路径
const outputPath = path.join(__dirname, 'australia_programs_new.csv');
...
if (!isMainThread) {
      ...
      try {
        ...
        official_website = await page.evaluate(() => {
          ...
          function isLikelyUniversityWebsite(url, schoolName) {
            // 澳大利亚大学域名通常包含.edu.au
            const eduDomains = ['.edu.au', '.unimelb.edu.au', '.sydney.edu.au', '.uq.edu.au', '.anu.edu.au', '.monash.edu', '.unsw.edu.au', '.uwa.edu.au', '.adelaide.edu.au', '.uts.edu.au', '.mq.edu.au'];
  ...
  // 获取项目URL列表
  async function getProjectUrls(baseUrl, totalPages = 44) {
    ...
    try {
      for (let pageNum = 1; pageNum <= totalPages; pageNum++) {
        // 正确构造指南者留学网站的URL格式：https://www.compassedu.hk/class_c7p{pageNum}
        // 直接使用完整的URL格式
        const url = `${baseUrl.replace(/p\d+$/, '')}p${pageNum}`;
  ...
  // 主函数
  async function main() {
    console.log('开始爬取美国硕士项目数据（多线程版 - 即时保存）...');
    ...
    try {
      ...
      // 获取项目URL列表，爬取全部222页
      // 正确的基础URL: https://www.compassedu.hk/class_c7p1
      const projectUrls = await getProjectUrls('https://www.compassedu.hk/class_c9p1', 44);
