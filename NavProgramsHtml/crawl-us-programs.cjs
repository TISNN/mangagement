...
// 设置CSV文件保存路径
const outputPath = path.join(__dirname, 'us_programs_new.csv');
...
if (!isMainThread) {
      ...
      try {
        ...
        official_website = await page.evaluate(() => {
          ...
          function isLikelyUniversityWebsite(url, schoolName) {
            // 美国大学域名通常包含.edu
            const eduDomains = ['.edu', '.harvard.edu', '.mit.edu', '.stanford.edu', '.berkeley.edu', '.ucla.edu', '.umich.edu', '.columbia.edu', '.yale.edu', '.princeton.edu', '.cornell.edu'];
  ...
  // 获取项目URL列表
  async function getProjectUrls(baseUrl, totalPages = 112) {
  ...
  async function main() {
    ...
    try {
      ...
      const projectUrls = await getProjectUrls('https://www.compassedu.hk/class_c12p1', 112);
