...
// 设置CSV文件保存路径
const outputPath = path.join(__dirname, 'uk_programs_new.csv');
...
if (!isMainThread) {
  ...
  // 获取项目URL列表
  async function getProjectUrls(baseUrl, totalPages = 222) {
  ...
  // 主函数
  async function main() {
    console.log('开始爬取英国硕士项目数据（多线程版 - 即时保存）...');
    ...
    try {
      ...
      const projectUrls = await getProjectUrls('https://www.compassedu.hk/class_c7p1', 222);
